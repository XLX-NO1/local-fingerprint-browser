...
describe('buildDeviceProfileRows', () => {
  it('formats the profile fingerprint into readable device rows', () => {
    ...
    expect(rows).toContainEqual({ label: 'System Chip', value: 'Apple M2' });
  ...
  it('labels Intel Mac fingerprints as Intel instead of Apple Silicon', () => {
    const rows = buildDeviceProfileRows({
      os: 'macos',
      browserVersion: '126.0.0.0',
      userAgent: 'Mozilla/5.0 test',
    ...
    expect(rows).toContainEqual({ label: 'Platform', value: 'MacIntel' });
    expect(rows).toContainEqual({ label: 'System Chip', value: 'Intel Mac' });
  });
});
